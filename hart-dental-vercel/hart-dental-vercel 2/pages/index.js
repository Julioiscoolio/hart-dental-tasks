...
// Disable static generation — this app needs runtime env vars
export const getServerSideProps = async () => ({ props: {} });

// Create supabase client safely at runtime only
...
async function loadTasks() {
  const { data, error } = await getSupabase().from("dental_tasks").select("payload");
...
async function saveTasks(tasks) {
  ...
  const { error } = await getSupabase().from("dental_tasks").upsert(rows, { onConflict: "id" });
...
async function loadRecurring() {
  const { data, error } = await getSupabase().from("dental_recurring").select("payload");
  if (error) return [];
  return (data || []).map(r => r.payload);
}
...
async function saveRecurring(templates) {
  ...
  await getSupabase().from("dental_recurring").upsert(rows, { onConflict: "id" });
}
